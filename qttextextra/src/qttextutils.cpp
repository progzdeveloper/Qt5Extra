...
namespace Qt5Extra
{
    ...
    QStringList systemLanguagesHumanReadable()
    {
        ...
        {
            QStringList res;
            const auto langs = QLocale::system().uiLanguages();
            res.reserve(langs.size());

            for (const auto& tag : langs)
                res.push_back(tag.left(std::max(tag.indexOf(u'_'), tag.indexOf(u'-'))));
    ...
    QStringList systemLanguagesISO639()
    {
        const static auto languages = []()
        {
            auto langs = QLocale::system().uiLanguages();
            for (auto& tag : langs)
                tag = QLocale{tag}.name();
            return langs;
        }();
        return languages;
    }

    QStringList systemLanguagesBCP47()
    {
        const static auto languages = []()
        {
            auto langs = QLocale::system().uiLanguages();
            for (auto& tag : langs)
                tag = QLocale{tag}.bcp47Name();
            return langs;
        }();
        return languages;
    }
    ...
    QStringList systemLanguages(LangNameFormat format)
    {
        switch(format)
        {
        case LangNameFormat::LangFormat_HumanReadable:
            return systemLanguagesHumanReadable();
        case LangNameFormat::LangFormat_BCP47:
            return systemLanguagesBCP47();
        case LangNameFormat::LangFormat_ISO639:
        default:
            return systemLanguagesISO639();
        }
        return {}; // unreachable
    }
    ...
    QVector<QChar::Script> supportedScripts()
    {
        const static auto scripts = []()
        {
            ...
            {
                const auto s = localeToScriptCode(l);
                if (s == QChar::Script_Common)
                    continue;
                res.push_back(s);
            }
