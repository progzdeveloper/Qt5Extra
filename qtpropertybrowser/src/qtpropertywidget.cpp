...
void QtPropertyWidgetPrivate::initUi()
{
    QObject::connect(manager, &QtVariantPropertyManager::valueChanged,
                     q_ptr, &QtPropertyWidget::slotValueChanged);
...
QtPropertyWidget::QtPropertyWidget(QWidget *parent)
    : QWidget(parent), d(new QtPropertyWidgetPrivate(this))
{
    d->initUi();
}
...
void QtPropertyWidget::setupObject(QObject *object)
{
    ...
    if (d->object != Q_NULLPTR && !d->isGadget)
        disconnect(d->object, &QObject::destroyed, this, &QtPropertyWidget::objectDestroyed);
    ...
    connect(object, &QObject::destroyed, this, &QtPropertyWidget::objectDestroyed);
