    ...
    : QtAbstractPropertyBrowser(parent), d_ptr(new QtTreePropertyBrowserPrivate)
{
    ...
    d_ptr->init(this);
    connect(this, SIGNAL(currentItemChanged(QtBrowserItem*)),
...
void QtTreePropertyBrowser::setAnimated(bool on)
{
    return d_ptr->m_treeWidget->setAnimated(on);
}

bool QtTreePropertyBrowser::isAnimated() const
{
    return d_ptr->m_treeWidget->isAnimated();
}
...
void QtTreePropertyBrowser::setBackgroundColor(QtBrowserItem *item, const QColor &color)
{
    ...
    d_ptr->m_treeWidget->doItemsLayout();
}
