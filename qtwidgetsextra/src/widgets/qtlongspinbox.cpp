...
QtLongSpinBox::QtLongSpinBox(QWidget *parent) :
    QtLongSpinBox(0, 100, parent)
{
}

QtLongSpinBox::QtLongSpinBox(qlonglong minimum, qlonglong maximum, QWidget *parent)
    : QAbstractSpinBox(parent)
    , d(new QtLongSpinBoxPrivate(this))
{
    d->updateEdit();

    if (auto editor = lineEdit())
    {
        editor->setValidator(new QtLongSpinBoxValidator(this));
        connect(editor, &QLineEdit::textChanged, this, &QtLongSpinBox::slotEditorTextChanged);
    }
...
QtLongSpinBox::~QtLongSpinBox() = default;
