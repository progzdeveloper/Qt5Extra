...
void QtOverviewWidgetPrivate::grabViewport()
{
...

    {
        ...
        pixmap = viewport->grab(QRect(0, 0, width, height));
        if (!pixmap.isNull())
            pixmap = pixmap.scaled(q_ptr->size(), Qt::KeepAspectRatio, Qt::SmoothTransformation);
    }
...
QtOverviewWidget::QtOverviewWidget(QWidget *parent, Qt::WindowFlags flags)
    ...
    , d(new QtOverviewWidgetPrivate(this))
{
...
QtOverviewWidget::~QtOverviewWidget() = default;
