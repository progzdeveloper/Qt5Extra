...
class QtColorTrianglePrivate
{
public:
    QtColorTrianglePrivate(QWidget* w) :
        bg(w->sizeHint(), QImage::Format_RGB32), selMode(Idle)
    {
    }
    ...
    QPointF movePointToTriangle(double x, double y, const Vertex &a,
                                const Vertex &b, const Vertex &c, const QRect &rect) const;

    QPointF pointFromColor(const QColor &col) const;
    QColor colorFromPoint(const QPointF &p, const QRect &rect) const;

    void genBackground(QWidget *w);
...
QtColorTriangle::QtColorTriangle(QWidget *parent)
    : QWidget(parent)
    , d(new QtColorTrianglePrivate(this))
{
...
*/
QtColorTriangle::~QtColorTriangle() = default;
...
void QtColorTriangle::polish()
{
    ...
    d->outerRadius = (contentsRect().width() - 1) / 2;
    if ((contentsRect().height() - 1) / 2 < d->outerRadius)
        d->outerRadius = (contentsRect().height() - 1) / 2;

    d->penWidth = (int) floor(d->outerRadius / 50.0);
    d->ellipseSize = (int) floor(d->outerRadius / 12.5);

    double cx = (double) contentsRect().center().x();
    double cy = (double) contentsRect().center().y();

    double diff5 = (d->outerRadius - (d->outerRadius / 5.0));
    double diff10 = (d->outerRadius - (d->outerRadius / 10.0));

    d->pa = QPointF(cx + (cos(d->a) * diff5),
                    cy - (sin(d->a) * diff5));
    d->pb = QPointF(cx + (cos(d->b) * diff5),
                    cy - (sin(d->b) * diff5));
    d->pc = QPointF(cx + (cos(d->c) * diff5),
                    cy - (sin(d->c) * diff5));
    d->pd = QPointF(cx + (cos(d->a) * diff10),
                    cy - (sin(d->a) * diff10));

    // Find the current position of the selector
    d->selectorPos = d->pointFromColor(d->curColor);
...
/*! \reimp
 */
QSize QtColorTriangle::minimumSizeHint() const
{
...
*/
void QtColorTrianglePrivate::genBackground(QWidget* w)
{
    ...
    bg = QImage(w->contentsRect().size(), QImage::Format_RGB32);
    QPainter p(&bg);
    p.setRenderHint(QPainter::Antialiasing);
    p.fillRect(bg.rect(), w->palette().mid());
...
void QtColorTriangle::mouseMoveEvent(QMouseEvent *e)
{
    ...
    if (d->selMode == QtColorTrianglePrivate::SelectingHue) {
        // If selecting hue, find the new angles for the points a,b,c
        // of the triangle. The following update() will then redraw
        // the triangle.
        d->a = d->angleAt(depos, contentsRect());
        d->b = d->a + TWOPI / 3.0;
        d->c = d->b + TWOPI / 3.0;
        if (d->b > TWOPI) d->b -= TWOPI;
        if (d->c > TWOPI) d->c -= TWOPI;

        double am = d->a - PI/2;
        if (am < 0) am += TWOPI;

        d->curHue = 360 - (int) (((am) * 360.0) / TWOPI);
        int h,s,v;
        d->curColor.getHsv(&h, &s, &v);

        if (d->curHue != h) {
            newColor = true;
            d->curColor.setHsv(d->curHue, s, v);
        }
        ...
        Vertex aa(Qt::black, d->pa);
        Vertex bb(Qt::black, d->pb);
        Vertex cc(Qt::black, d->pc);
        ...
        d->selectorPos = d->movePointToTriangle(depos.x(), depos.y(), aa, bb, cc, contentsRect());
        QColor col = d->colorFromPoint(d->selectorPos, contentsRect());
        if (col != d->curColor) {
...
void QtColorTriangle::mousePressEvent(QMouseEvent *e)
{
    ...
    QPointF depos((double) e->pos().x(), (double) e->pos().y());
    double rad = d->radiusAt(depos, contentsRect());
    bool newColor = false;

    // As in mouseMoveEvent, either find the a,b,c angles or the
    // radian position of the selector, then order an update.
    if (rad > (d->outerRadius - (d->outerRadius / 5))) {
        d->selMode = QtColorTrianglePrivate::SelectingHue;
        ...
        d->selMode = QtColorTrianglePrivate::SelectingSatValue;
...
void QtColorTriangle::mouseReleaseEvent(QMouseEvent *e)
{
    ...
    if (e->button() == Qt::LeftButton)
        d->selMode = QtColorTrianglePrivate::Idle;
}
...
void QtColorTriangle::keyPressEvent(QKeyEvent *e)
{
    ...
    case Qt::Key_Left: {
        --(d->curHue);
        if (d->curHue < 0) d->curHue += 360;
        int h,s,v;
        d->curColor.getHsv(&h, &s, &v);
        QColor tmp;
        tmp.setHsv(d->curHue, s, v);
        setColor(tmp);
    }
        break;
    case Qt::Key_Right: {
        ++(d->curHue);
        if (d->curHue > 359) d->curHue -= 360;
        int h,s,v;
        d->curColor.getHsv(&h, &s, &v);
        QColor tmp;
        tmp.setHsv(d->curHue, s, v);
...
void QtColorTriangle::paintEvent(QPaintEvent *e)
{
    ...
    QColor hueColor;
    hueColor.setHsv(d->curHue, 255, 255);

    // Draw the triangle
    d->drawTrigon(&buf, d->pa, d->pb, d->pc, hueColor);
    ...
    painter.drawLine(d->pa, d->pb);
    painter.drawLine(d->pb, d->pc);
    painter.drawLine(d->pc, d->pa);
    
    int ri, gi, bi;
    hueColor.getRgb(&ri, &gi, &bi);
    if ((ri * 30) + (gi * 59) + (bi * 11) > 12800)
        painter.setPen(QPen(Qt::black, d->penWidth));
    else
        painter.setPen(QPen(Qt::white, d->penWidth));
    painter.drawEllipse((int) (d->pd.x() - d->ellipseSize / 2.0),
                        (int) (d->pd.y() - d->ellipseSize / 2.0),
                        d->ellipseSize, d->ellipseSize);

    d->curColor.getRgb(&ri, &gi, &bi);
    ...
    painter.drawEllipse(QRectF(d->selectorPos.x() - d->ellipseSize / 2.0,
                               d->selectorPos.y() - d->ellipseSize / 2.0,
                               d->ellipseSize + 0.5, d->ellipseSize + 0.5));
...
Draws a trigon (polygon with three corners \a pa, \a pb and \a pc
    ...
    gradient.
*/
void QtColorTrianglePrivate::drawTrigon(QImage *buf, const QPointF &pa,
...
void QtColorTriangle::setColor(const QColor &col)
{
    ...
    d->a = (((360 - d->curHue) * TWOPI) / 360.0);
    d->a += PI / 2.0;
    if (d->a > TWOPI) d->a -= TWOPI;
    ...
    double cx = (double) contentsRect().center().x();
    double cy = (double) contentsRect().center().y();
    double innerRadius = d->outerRadius - (d->outerRadius / 5.0);
    double pointerRadius = d->outerRadius - (d->outerRadius / 10.0);

    d->pa = QPointF(cx + (cos(d->a) * innerRadius), cy - (sin(d->a) * innerRadius));
    d->pb = QPointF(cx + (cos(d->b) * innerRadius), cy - (sin(d->b) * innerRadius));
    d->pc = QPointF(cx + (cos(d->c) * innerRadius), cy - (sin(d->c) * innerRadius));
    d->pd = QPointF(cx + (cos(d->a) * pointerRadius), cy - (sin(d->a) * pointerRadius));
...
*/
double QtColorTrianglePrivate::radiusAt(const QPointF &pos, const QRect &rect) const
{
...
*/
double QtColorTrianglePrivate::angleAt(const QPointF &pos, const QRect &rect) const
{
    double mousexdist = pos.x() - (double) rect.center().x();
    double mouseydist = pos.y() - (double) rect.center().y();
...
*/
QPointF QtColorTrianglePrivate::movePointToTriangle(double x, double y, const Vertex &a,
                                             const Vertex &b, const Vertex &c, const QRect& rect) const
{
    ...
    double angleA = angleAt(a.point, rect);
    double angleB = angleAt(b.point, rect);
    double angleC = angleAt(c.point, rect);
    double angleP = angleAt(QPointF(x, y), rect);
...
/*! \internal
    ...
    and value as \a col.
*/
QPointF QtColorTrianglePrivate::pointFromColor(const QColor &col) const
{
...
/*! \internal
    ...
    these two colors.
*/
QColor QtColorTrianglePrivate::colorFromPoint(const QPointF &p, const QRect& rect) const
{
    // Find the outer radius of the hue gradient.
    int outerRadius = (rect.width() - 1) / 2;
    if ((rect.height() - 1) / 2 < outerRadius)
        outerRadius = (rect.height() - 1) / 2;

    // Find the center coordinates
    double cx = (double) rect.center().x();
    double cy = (double) rect.center().y();

    double diff5 = (outerRadius - (outerRadius / 5.0));

    // Find the a, b and c from their angles, the center of the rect
    // and the radius of the hue gradient donut.
    QPointF pa(cx + (cos(a) * diff5),
               cy - (sin(a) * diff5));
    QPointF pb(cx + (cos(b) * diff5),
               cy - (sin(b) * diff5));
    QPointF pc(cx + (cos(c) * diff5),
               cy - (sin(c) * diff5));
