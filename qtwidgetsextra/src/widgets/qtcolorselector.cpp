...
QtColorSelector::QtColorSelector(QWidget *parent)
    : QWidget(parent)
    , d(new QtColorSelectorPrivate(this))
{
...
QtColorSelector::~QtColorSelector() = default;
...
void QtColorSelector::swapColors()
{
    ...
    Q_EMIT backgroundChanged(d->bg);
    Q_EMIT foregroundChanged(d->fg);
...
void QtColorSelector::mousePressEvent(QMouseEvent *ev)
{
    ...
    if (ev->button() == Qt::LeftButton)
    {
        if (d->foregroundRect().contains(ev->pos())) {
            d->fgP = true;
            d->bgP = false;
            Q_EMIT foregroundPressed();
            update();
        } else if (d->backgroundRect().contains(ev->pos())) {
            d->bgP = true;
            d->fgP = false;
            Q_EMIT backgroundPressed();
            update();
        }
    }
}
