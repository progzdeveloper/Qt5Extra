...
public Q_SLOTS:
    ...
    virtual void moveNext() { setCurrent(current() + 1); }

    virtual void movePrev() { setCurrent(current() - 1); }

    virtual void moveFirst() { setCurrent(minimum()); }

    virtual void moveLast() { setCurrent(maximum()); }
...
private:
    friend class QtRangeNavigatorPrivate;
    QScopedPointer<class QtRangeNavigatorPrivate> d;
};
