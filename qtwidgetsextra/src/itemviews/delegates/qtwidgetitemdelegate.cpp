...
QString uniqueName(const QModelIndex& index, const QSize& size)
{
#ifdef QT_DEBUG
    constexpr size_t kMaxBufSize = 256;
    constexpr const char* fmt = "%i:%i_%ix%i";
    QByteArray readable;
    const int n = qsnprintf(nullptr, kMaxBufSize, fmt, index.row(), index.column(), size.width(), size.height());
    readable.reserve(n);
...
void QtWidgetItemDelegatePrivate::renderCached(QPainter *painter, const QStyleOptionViewItem& option, const QModelIndex& index, const QtWidgetItemDelegate *delegate) const
{
    ...
    const QString hashCode = uniqueName(index, option.rect.size());
...
QtWidgetItemDelegate::QtWidgetItemDelegate(QObject *parent)
    ...
    , d(new QtWidgetItemDelegatePrivate)
{
     
    d->cache.setCacheLimit(512);
}

QtWidgetItemDelegate::~QtWidgetItemDelegate() = default;
...
void QtWidgetItemDelegate::invalidateIndex(const QModelIndex &index)
{
    ...
    if (view)
        d->cache.remove(uniqueName(index, view->visualRect(index).size()));
}
...
void QtWidgetItemDelegate::invalidateRange(const QModelIndex &topLeft, const QModelIndex &bottomRight)
{
    ...
    for (int i = topLeft.row(), n = bottomRight.row(); i <= n; ++i)
        for (int j = topLeft.column(), m = bottomRight.column(); j <= m; ++j)
            d->cache.remove(uniqueName(index.sibling(i, j), view->visualRect(index).size()));
}
