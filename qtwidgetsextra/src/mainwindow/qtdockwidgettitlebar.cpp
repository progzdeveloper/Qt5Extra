...
public:
    ...
    inline QToolButton *createButton(QtDockWidgetButton type, bool autoHide, QSignalMapper *mapper) const;
...
void QtDockWidgetTitleBarPrivate::createButtons(QtDockWidget *parent)
{
    ...
    for (int i = 0; i <= CloseButton; ++i)
        buttons[i] = createButton(static_cast<QtDockWidgetButton>(i), parent->isAutoHide(), signalMapper);

    QObject::connect(signalMapper, qOverload<int>(&QSignalMapper::mapped), q_ptr, &QtDockWidgetTitleBar::clicked);
    buttons[UnshadeButton]->setMenu(createMenu(parent));
    buttons[UnshadeButton]->setPopupMode(QToolButton::InstantPopup);
}

QToolButton *QtDockWidgetTitleBarPrivate::createButton(QtDockWidgetButton type, bool autoHide, QSignalMapper* mapper) const
{
    ...
    QObject::connect(button, &QToolButton::clicked, mapper, qOverload<>(&QSignalMapper::map));
    return button;
}
...
void QtDockWidgetTitleBarPrivate::createTitleBar(QtDockWidget *dw)
{
    ...
    createButtons(dw);
    createLayout(dw);
    QObject::connect(dw, &QtDockWidget::featuresChanged, q_ptr, &QtDockWidgetTitleBar::changeFeatures);
    QObject::connect(dw, &QtDockWidget::autoHideChanged, q_ptr, &QtDockWidgetTitleBar::updateButtons);
}
...
QtDockWidgetTitleBar::QtDockWidgetTitleBar( QWidget *parent /*= 0*/, Qt::WindowFlags f /*= 0*/) 
    : QWidget(parent, f), d(new QtDockWidgetTitleBarPrivate)
{
    d->q_ptr = this;
    d->createTitleBar(qobject_cast<QtDockWidget*>(parent));
    d->createPalette();
    setMouseTracking(true);
}

QtDockWidgetTitleBar::~QtDockWidgetTitleBar() = default;
